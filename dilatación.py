...
th,imbinary = cv2.threshold(im_gray, 130, 255, cv2.THRESH_BINARY)
...
dilatacion = cv2.dilate(imbinary,kernel,iterations = 1)
...
plt.subplot(132),plt.imshow(imbinary, cmap = 'gray')
plt.title('Binaria'), plt.xticks([]), plt.yticks([])


plt.subplot(133),plt.imshow(dilatacion, cmap = 'gray')
